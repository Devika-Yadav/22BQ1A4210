  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const mockResponse = validInputs.map((item, i) => ({
        ...
        shortcode: item.shortcode || "autogen" + Math.random().toString(36).slice(2, 8),
        expiry: new Date(Date.now() + item.validity * 60000).toLocaleString(),
      }));
        ...
        <Box>
          ...
          {results.map((res, idx) => (
            <Alert key={idx} severity="success" sx={{ mt: 2 }}>
              <strong>{res.originalUrl}</strong> → http://localhost:3000/{res.shortcode} (expires at {res.expiry})
            </Alert>
          ))}
        </Box>
      )}
